...
module.exports = function(group, element, bpmnFactory) {
  ...
  group.entries.push({
    ...
    html: '<div class="pp-row">' +
            '<label for="camunda-form-type">表单类型</label>' +
            '<div class="pp-field-wrapper">' +
              '<select id="camunda-form-type" name="formType" data-value>' +
                '<option value="form-key">链接</option>' +
                '<option value="form-data">自定义</option>' +
              '</select>' +
            '</div>' +
          '</div>',
    get: function(element, node) {
    ...
    set: function(element, values, node) {
      ...
      if (values.formType === 'form-key') {
        ...
        var entry = getExtensionElements(bo, 'runbpm:FormData');
        ...
        commands.push(cmdHelper.updateBusinessObject(element, bo, { 'runbpm:formKey': undefined }));
        ...
        var formData = elementHelper.createElement('runbpm:FormData', { fields: [] }, extensionElements, bpmnFactory);
  ...
  group.entries.push(entryFactory.textField({
    ...
    set: function(element, values, node) {
      ...
      return cmdHelper.updateBusinessObject(element, bo, { 'runbpm:formKey': formKey });
    },
    disabled: function(element, node) {
  ...
  var formFieldsEntry = extensionElements(element, bpmnFactory, {
    ...
    createExtensionElement: function(element, extensionElements, value){
      var bo = getBusinessObject(element),
          formData = getExtensionElements(bo, 'runbpm:FormData')[0],
          field = elementHelper.createElement('runbpm:FormField', { id: value }, formData, bpmnFactory);
    ...
    removeExtensionElement: function(element, extensionElements, value, idx) {
      var formData = getExtensionElements(getBusinessObject(element), 'runbpm:FormData')[0],
  ...
  group.entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      ...
      var enumValue = elementHelper.createElement('runbpm:Value', { id: id, name: undefined },
  ...
  group.entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      ...
      if (!validation) {
        ...
        validation = elementHelper.createElement('runbpm:Validation', {}, getBusinessObject(element), bpmnFactory);

        commands.push(cmdHelper.updateBusinessObject(element, formField, { 'validation': validation }));
      }

      var newConstraint = elementHelper.createElement('runbpm:Constraint',
