...
var is = require('bpmn-js/lib/util/ModelUtil').is,
  ...
  script = require('./implementation/RunBPMConditionExpression')('runbpm:advancedType', 'body');
...
module.exports = function(group, element, bpmnFactory) {
  ...
  group.entries.push({
    ...
    'html': '<div class="pp-row">' +
              '<label for="cam-condition-type">类型</label>' +
              '<div class="pp-field-wrapper">' +
                '<select id="cam-condition-type" name="conditionType" data-value>' +
                  '<option value="expression">表达式</option>' +
                  '<option value="script">高级类型</option>' +
                  '<option value="" selected>永真</option>' +
                '</select>' +
              '</div>' +
            '</div>' +
    ...
    get: function (element, propertyName) {
      ...
      if(conditionExpression) {
        var conditionLanguage = conditionExpression.advancedType;
