...
var attributeInfo = {
  bpmn: {
    ...
    binding: 'runbpm:calledElementBinding',
    version: 'runbpm:calledElementVersion'
  },

  cmmn: {
    element: 'runbpm:caseRef',
    binding: 'runbpm:caseBinding',
    version: 'runbpm:caseVersion'
  },

  dmn: {
    element: 'runbpm:decisionRef',
    binding: 'runbpm:decisionRefBinding',
    version: 'runbpm:decisionRefVersion'
  }
};
...
function getCamundaInWithBusinessKey(element) {
  ...
  var camundaInParams = extensionElementsHelper.getExtensionElements(bo, 'runbpm:In');
  if (camundaInParams) {
...
module.exports = function (element, bpmnFactory, options) {
  ...
  entries.push(entryFactory.selectBox({
    ...
    get: function(element, node) {
      var bo = getBusinessObject(element);
      return {
        mapDecisionResult: bo.get('runbpm:mapDecisionResult') || 'resultList'
      };
    },

    set: function(element, values, node) {
      return cmdHelper.updateProperties(element, {
        'runbpm:mapDecisionResult': values.mapDecisionResult || 'resultList'
      });
    },

    disabled: function(element, node) {
      var bo = getBusinessObject(element);
      var resultVariable = bo.get('runbpm:resultVariable');
